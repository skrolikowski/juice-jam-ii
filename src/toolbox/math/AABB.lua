...
local function nearest(aabb, point)
    local t = {
        -- x-checks
        { d = math.abs(point.x - aabb.left), p = Vec2(aabb.left, point.y) },
        { d = math.abs(aabb.right - point.x), p = Vec2(aabb.right, point.y) },
        -- y-checks
        { d = math.abs(point.y - aabb.top), p = Vec2(point.x, aabb.top) },
        { d = math.abs(aabb.bottom - point.y), p = Vec2(point.x, aabb.bottom) }
    }
...
function AABB:center()
    return self.left + math.floor(self:width() / 2),
        self.top + math.floor(self:height() / 2)
...
function AABB:translate(tx, ty)
    if not type(tx) == "number" and tx.__name == 'Vec2' then
        tx, ty = tx:unpack()
    end
...
function AABB:compute(cx, cy, width, height, rotation)
    ...
    local ex = math.max(x1, x2, x3, x4)
    local ey = math.max(y1, y2, y3, y4)
